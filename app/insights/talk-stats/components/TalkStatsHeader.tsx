...
import { P, PERIOD_OPTIONS } from "@/lib/talk-stats/data";
import { useOverviewCampaigns } from "@/hooks/use-reach-overview";

interface TalkStatsHeaderProps {
...
export function TalkStatsHeader({
  activeView,
  selectedCampaigns,
  selectedPeriod,
  customDateRange,
  ...
  const apiCampaigns = useMemo(() => campaignsData?.campaigns ?? [], [campaignsData]);

  useEffect(() => {
    if (
      apiCampaigns.length > 0 &&
      (selectedCampaigns[0] === "all" || selectedCampaigns.length === 0)
    ) {
      onCampaignChange([apiCampaigns[0].id]);
    }
  }, [apiCampaigns, onCampaignChange, selectedCampaigns]);
  ...
  const campaignDisplayText = () => {
    ...
    if (selectedCampaigns.length === 1) {
      return apiCampaigns.find((campaign) => campaign.id === selectedCampaigns[0])?.name ?? "Campaign";
    }
  ...
  const periodLabel =
    selectedPeriod === "custom"
      ...
      : PERIOD_OPTIONS.find((period) => period.key === selectedPeriod)?.label;

  const goalLabel = activeView === "advocacy" ? "Advocacy goal" : "Retention goal";
    ...
    >
      <div style={{ maxWidth: 1360, margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ paddingLeft: 16, marginBottom: 4 }}>
          <button
            onClick={() => router.push("/insightsiq")}
            style={{
              fontSize: 11,
              fontWeight: 600,
        ...
        >
          <div>
            ...
            <p style={{ fontSize: 13, color: P.text3, marginTop: 6, fontWeight: 400 }}>
              ...
              {selectedPeriod === "custom"
                ? `${customDateRange.from} – ${customDateRange.to}`
                : `Last ${PERIOD_OPTIONS.find((period) => period.key === selectedPeriod)?.label}`}{" "}
              vs previous period
            </p>
          </div>
          <button
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div ref={campaignRef} style={{ position: "relative" }}>
              ...
              {showCampaignDropdown && (
                ...
                >
                  {apiCampaigns.length === 0 ? (
                    <div style={{ padding: "12px 14px", fontSize: 13, color: P.text3 }}>
                      Loading...
                    </div>
                  ) : (
                        ...
                        >
                          <span
                            style={{
                              ...
                              height: 6,
                              borderRadius: "50%",
                              background: campaign.status === "active" ? "#34D399" : P.text3,
                              flexShrink: 0,
                            }}
                          />
                          <span
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 2, marginLeft: 8 }}>
              {PERIOD_OPTIONS.filter((period) => period.key !== "custom").map((period) => (
                <button
                  key={period.key}
                  onClick={() => {
                  ...
                  style={{
                    ...
                    background: selectedPeriod === period.key ? P.accent : "transparent",
                    color: selectedPeriod === period.key ? "#fff" : P.text2,
                    borderRadius: 6,
                    cursor: "pointer",
                    transition: "all 0.2s",
                  }}
          ...
          <div style={{ fontSize: 12, color: P.text3 }}>
            Showing: <strong style={{ color: P.text2 }}>{campaignDisplayText()}</strong>{" "}
