...
import { P, CAMPAIGNS, PERIOD_OPTIONS } from "@/lib/act-stats/data";

interface ActStatsHeaderProps {
...
export function ActStatsHeader({
  selectedCampaigns,
  selectedPeriod,
  customDateRange,
  onCampaignChange,
  ...
  const nonAllCampaigns = CAMPAIGNS.filter((c) => c.id !== "all");
  ...
  const isCampaignSelected = (id: string) =>
    id === "all" ? selectedCampaigns[0] === "all" : selectedCampaigns[0] === id;

  const campaignDisplayText = () => {
    if (isAllSelected) return "All Campaigns";
    if (selectedCampaigns.length === 1)
      return CAMPAIGNS.find((c) => c.id === selectedCampaigns[0])?.name ?? "Campaign";
    return `${selectedCampaigns.length} Campaigns`;
  };

  const campaignSubText = () => {
    if (isAllSelected) return `${nonAllCampaigns.length} campaigns`;
    const ch = CAMPAIGNS.find((c) => c.id === selectedCampaigns[0])?.channels;
                    ...
                    return (
                      ...
                      >
                        ...
                        <span style={{ fontSize: 11, color: P.text3 }}>
                          {c.id === "all" ? `${CAMPAIGNS.length - 1}` : `${c.channels} ch.`}
                        </span>
                        {selected && (
